...
export default function ModalTakeInterest({
  currentAccount,
  open,
  setOpen,
  callback,
  ...
  return (
    <Modal
      ...
      visible={open}
      title="Xác nhận rút tiền lãi tiết kiệm"
      width={700}
      footer={null}
    >
      <SStyle>
        ...
        <div className="btn-container">
          ...
          <Button
            ...
            onClick={() => {
              requestToken({
                method: "POST",
                url: "/account/withdraw-saving-interest",
                data: { id: currentAccount?.id },
              })
                .then((res) => {
                  Alert({ name: "Rút tiền lãi thành công!", icon: "success" });
                  setOpen(false);
                  callback && callback();
                })
                .catch((err) => {
